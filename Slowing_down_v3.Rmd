...
```{r oxygen_plot, fig.cap='Oxygen concetration dynamics over time. The red areas show the three heatwaves. The figure shows the difference in DO between mesocosm undergoing the HWs treatment and the control (dashed line at zero).', fig.align="center", fig.height=6, fig.width=10}
...
```{r oxygen_slopes, fig.cap='Change in resilience of ecosystem respiration after each HW.', fig.align="center", fig.height=6, fig.width=10}
...
ox_combined <- ox_plot / p_resilience_ox
ox_combined
```
...
days_of_interest <- unique(ox_mod$Day)  # You can also specify specific days manually, e.g., c(1, 5, 10, 15)
...
```{r chla_slopes, fig.cap='Change in resilience of chlorophyll - a concentration after each HW.', fig.align="center", fig.height=6, fig.width=10}
# Calculate mean abundance and standard error for Control
control_mean <- chla %>% filter(Treatment == "Control") %>%
  group_by(Day) %>%
  summarize(control_mean_chla = mean(chla, na.rm = TRUE))
...
resiliene_HW1 <- tot.chla_diff %>% filter(Day %in% c(-4, 10),
                                  Treatment == "HW") %>% 
  group_by(Cosm) %>% 
  mutate(resilience = (diff_chla[Day == -4] / diff_chla[Day == 10])) %>% 
  filter(Day == "-4")


resiliene_HW2 <- tot.chla_diff %>% filter(Day %in% c(10, 24),
                                   Treatment == "HW") %>% 
  group_by(Cosm) %>% 
  mutate(resilience = (diff_chla[Day == 10] / diff_chla[Day == 24])) %>% 
  filter(Day == 24)



resiliene_HW3 <- tot.chla_diff %>% filter(Day %in% c(24, 38),
                                   Treatment == "HW") %>% 
  group_by(Cosm) %>% 
  mutate(resilience = abs(diff_chla[Day == 24] / diff_chla[Day == 38])) %>% 
  filter(Day == 38)


resilience <- rbind(resiliene_HW1, resiliene_HW2, resiliene_HW3)
...
p_resilience_chla <- ggplot(resilience, aes(x = heat, y = log10(resilience), group = Day)) +
  geom_boxplot() +
  geom_point(aes(color = as.factor(Cosm)), position = position_jitterdodge(), size = 3) +
  labs(x = "Heatwave", y = "log10(Resilience)", tag = "(b)") +
  theme_minimal(base_size = 20) +
...
chla_combined <- chla_plot / p_resilience_chla
chla_combined
```
...
# Plot the data with faceting and aligned y-axis
bio_plot_phyto <- ggplot(tot_bio_dynamics_filtered, aes(x = Day, y = mean_diff_bio, color = Treatment, fill = Treatment)) +
 geom_line(aes(group = Treatment), size = 1, color = "blue") +
  geom_point(size = 2, color = "blue") +
  geom_ribbon(aes(ymin = lower_y, ymax = upper_y, group = Treatment), alpha = 0.1, show.legend = FALSE, fill = "blue", color = "blue") + labs(tag = "(a)") +
  ylab("Phytoplankton \nDifference \nin log10(biomass)") +
  xlab("Day") +
  viridis::scale_colour_viridis(name = "Treatment", discrete = TRUE, end = 0.8, begin = 0.1, option = "inferno") +
  viridis::scale_fill_viridis(name = "Treatment", discrete = TRUE, end = 0.8, begin = 0.1, option = "inferno") +
...
```{r phyto_bio_plot, fig.cap='Phytoplankton biomass dynamics over time. The red areas show the three heatwaves', fig.align="center", fig.height=6, fig.width=10}
...
```{r phyto_bio_slopes, fig.cap='Change in the resilience of phytoplankton biomass after each HW.', fig.align="center", fig.height=6, fig.width=10}
try <- group_bio2[, -c(6,7)] %>% group_by(Sample, Treatment, Day) %>% summarize(total = sum(total_biomass)) %>% ungroup() 
try <- try %>% mutate(bio_log = log10(total))
# Calculate mean abundance and standard error for Control
control_mean <- try %>% filter(Treatment == "Control") %>%
...
resilience_HW1 <- tot.bio_diff %>% filter(Day %in% c(-4, 10),
                                    Treatment == "HW") %>% 
  group_by(Sample) %>% 
  mutate(resilience = abs(diff_bio[Day == -4] / diff_bio[Day == 10])) %>% 
  filter(Day == -4)


resilience_HW2 <- tot.bio_diff %>% filter(Day %in% c(10, 24),
                                     Treatment == "HW") %>% 
  group_by(Sample) %>% 
  mutate(resilience = abs(diff_bio[Day == 10] / diff_bio[Day == 24])) %>% 
  filter(Day == 24)



resilience_HW3 <- tot.bio_diff %>% filter(Day %in% c(24, 38),
                                     Treatment == "HW") %>% 
  group_by(Sample) %>% 
  mutate(resilience = abs(diff_bio[Day == 24] / diff_bio[Day == 38])) %>% 
  filter(Day == 38)
...
p_resilience_phyto <- ggplot(resilience, aes(x = heat, y = log10(resilience), group = Day)) +
  geom_boxplot() +
...
```{r phyto_combined, fig.cap='combined plot for manuscript', fig.align="center", fig.height=10, fig.width=12}
phyto_combined <- bio_plot_phyto / p_resilience_phyto
phyto_combined
```
...
```{r}
# Create a tidy summary
tidy_summary <- tidy(model_phyto_bio)

# Calculate 95% confidence intervals
conf_intervals <- confint(model_phyto_bio, level = 0.95)
...
doc <- read_docx()
doc <- body_add_flextable(doc, value = ft)
print(doc, target = "Tables/phyto_bio_model_summary.docx")
```
...
```{r}
# Specify the days you are interested in for post-hoc comparisons
days_of_interest <- unique(try$Day)  # You can also specify specific days manually, e.g., c(1, 5, 10, 15)

# Perform post-hoc analysis with specified days
emm_bio <- emmeans(model_phyto_bio, ~ Treatment | Day, at = list(Day = days_of_interest))
post_hoc_results_bio <- pairs(emm_bio)
# Convert post_hoc_results to a data frame
post_hoc_df_bio <- as.data.frame(post_hoc_results_bio)

# Order the results by the 'Day' column
post_hoc_df_bio <- post_hoc_df_bio %>% arrange(Day)

# Print the results using kable with limited number of decimals and additional formatting
kable(post_hoc_df_bio %>% mutate_if(is.numeric, ~round(., 3)), format = "html") %>%
  kable_styling(full_width = FALSE) %>%
  column_spec(1, bold = TRUE)  %>% 
  column_spec(7, bold = ifelse(post_hoc_df_chla$p.value < 0.05, TRUE, FALSE))
```
...
post_hoc_df_bio <- post_hoc_df_bio %>%
  mutate(across(where(is.numeric), ~ round(., 3)))
# Apply formatting to p-value column
post_hoc_df_bio <- post_hoc_df_bio %>%
  mutate(p_value = ifelse(p.value < 0.05, paste0("<b>", round(p.value, 3), "</b>"), round(p.value, 3)))
...
doc <- read_docx()
doc <- body_add_flextable(doc, value = ft)
print(doc, target = "Tables/Phyto_post_table_output.docx")

```

Model diagnostics
```{r check_phyto_bio_model, fig.cap= 'Checking assumption for chlorophyll - a model', fig.align="center", fig.height=12, fig.width=10}

check_model(model_phyto_bio)
```
Not too bad, but homogeneity of variance a bit messy. Consider transformation.
...
# Perform post-hoc analysis with specified days
emm_phyto_bio <- emmeans(model_phyto_bio, ~ Treatment | Day, at = list(Day = days_of_interest))
post_hoc_results_phyto_bio <- pairs(emm_phyto_bio)
# Convert post_hoc_results to a data frame
post_hoc_df_phyto_bio <- as.data.frame(post_hoc_results_phyto_bio)

# Order the results by the 'Day' column
post_hoc_df_phyto_bio <- post_hoc_df_phyto_bio %>% arrange(Day)

# Print the results using kable with limited number of decimals and additional formatting
kable(post_hoc_df_phyto_bio %>% mutate_if(is.numeric, ~round(., 3)), format = "html") %>%
  kable_styling(full_width = FALSE) %>%
  column_spec(1, bold = TRUE)  %>% 
  column_spec(7, bold = ifelse(post_hoc_df_phyto_bio$p.value < 0.05, TRUE, FALSE))
```
...
post_hoc_df_phyto_bio <- post_hoc_df_phyto_bio %>%
  mutate(across(where(is.numeric), ~ round(., 3)))
# Apply formatting to p-value column
post_hoc_df_phyto_bio <- post_hoc_df_phyto_bio %>%
  mutate(p_value = ifelse(p.value < 0.05, paste0("<b>", round(p.value, 3), "</b>"), round(p.value, 3)))
...
# Create a flextable object from the data frame
ft <- flextable(post_hoc_df_phyto_bio)
...
doc <- read_docx()
doc <- body_add_flextable(doc, value = ft)
print(doc, target = "Tables/phyto_bio_table_output.docx")
...
group_bio2$Day <- factor(group_bio2$Day, levels = c(-4, 3, 10, 15, 24, 30, 38))  # Specify custom order

# Step 1: Create an ordered factor version of Day just for plotting
group_bio2 <- group_bio2 %>%
  mutate(Day_flag = ifelse(Day %in% c(3, 15, 30), "highlight", "normal"))  # Create ordered factor for plotting



# Step 2: Build the plot
p_phyto <- ggplot(group_bio2, aes(x = Treatment, y = relative_bio, fill = group)) +
...
NMDS <- ggplot(combined_ordination_df, aes(x = MDS1, y = MDS2, fill = Treatment, group = interaction(Treatment, Day))) +
...
```{r bio_plot_zoo, fig.cap='Zooplankton biomass dynamics over time. The red areas show the three heatwaves', fig.align="center", fig.height=6, fig.width=10}
...
```{r}
# Create a tidy summary
tidy_summary <- tidy(model_zoo_bio)

# Calculate 95% confidence intervals
conf_intervals <- confint(model_zoo_bio, level = 0.95)
...
```{r relative_bio_zoo_plot, fig.cap='Change in relative biomass of zooplankton groups over time in the control and heatwave treatment', fig.align="center", fig.height=12, fig.width=10}
...
```{r rcombined_zoop, fig.cap='combined plot', fig.align="center", fig.height=15, fig.width=14}
