...
```{r phyto_bio_slopes, fig.cap='Change in the slope of the linear regression connecting two subsequent time point (during and after a HW) of the difference between phytoplankton biomass in HW and control mesocosms.', fig.align="center", fig.height=6, fig.width=10}
try <- try %>% mutate(total = log10(total))
# Calculate mean chla for Control by day
control_bm <- try %>% 
  filter(Treatment == "Control") %>%
  group_by(Day) %>% 
  dplyr::summarize(mean_bio_control = mean(total, na.rm = TRUE))

# Calculate mean chla for all treatments by day and cosm
bio_all_treat <- try %>% 
  group_by(Day, Sample, Treatment) %>% 
  dplyr::summarize(mean_bio = mean(total, na.rm = TRUE))

# Merge the control and treatment data
bio_merged <- merge(bio_all_treat, control_bm, by = "Day")
...
# Calculate the difference between HW and control
bio_merged <- bio_merged %>% 
  mutate(diff = mean_bio - mean_bio_control)

# Filter HW data
hw_data <- bio_merged %>% filter(Treatment == "HW")
...
# Create a combined plot
combined_plot <- ggplot(hw_data, aes(x = Day, y = diff)) +
  geom_point(aes(color = as.factor(Sample)), size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Day", y = "Difference (HW - Control)") +
  theme_bw(base_size = 22) +
  theme(legend.position = "none") +
...
```{r relative_bio_zoo_plot, fig.cap='Change in relative biomass of zooplankton groups over time in the control and heatwave treatment', fig.align="center", fig.height=20, fig.width=25}
